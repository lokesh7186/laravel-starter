<x-admin-layout :pageTitle="__('admin.new_user')">
    ...
    <form method="GET" action="{{ route('admin.user_permissions.search', ['username' => 'user-place-holder']) }}"
        id="selectUser">
        <x-admin.card :title="__('Select a User')" class="card-primary">

            <x-admin.input name="username" value="{{ old('username', $username) }}" label="{{ __('general.username') }}"
                id="searchUsername" />
                    ...
                    @forelse($allPermissions as $permission)
                        <div class="col-md-4 col-sm-6 col-lg-3">
                            <x-admin.checkbox value="{{ $permission->id }}" label="{{ $permission->name }}"
                                name="user_permissions[{{$loop->iteration}}]['name']" id="p-{{ $permission->id }}" inline="true"
                                checked="{{ $userPermissions->contains('id', $permission->id) }}" />
                        </div>
                    @empty
